...
- 首頁頁尾有「關於／隱私／聯絡」與 `/.well-known/security.txt`，聯絡信箱來自 `CONTACT_EMAIL`。
- **有人來信要求不被檢測**：到 Render 環境變數 `EXCLUDED_HOSTS` 加上該網域（逗號分隔，會連子網域一起擋），儲存後自動重新部署，之後掃該網域會回「擁有者已要求不被本工具檢測」。
- 這個網站自己也過得了自己的體檢：HSTS、CSP（script-src 沒有 'unsafe-inline'，JS 都在 static/）、COOP、security.txt 都有，測試裡有一條 `test_own_site_hygiene` 確保以後不會退步。
